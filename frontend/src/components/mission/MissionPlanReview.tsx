...
export function MissionPlanReview({ onClose }: MissionPlanReviewProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-[540px] max-h-[80vh] overflow-y-auto p-6 space-y-5 shadow-2xl shadow-black/50">
        <div className="flex justify-between items-center">
          <h2 className="text-sm font-semibold text-zinc-100">Mission Plan Review</h2>
          <button onClick={handleReject} className="text-zinc-500 hover:text-zinc-300 text-lg transition-colors">&times;</button>
        </div>
        ...
        {plan && (
          <>
            <div className="space-y-1">
              <div className="text-[13px] text-zinc-200 font-medium">{plan.name}</div>
            ...
            <div className="space-y-2.5">
              <div className="text-[11px] font-semibold text-zinc-400 uppercase tracking-wider">Assignments</div>
              {plan.assignments.map((a, i) => (
                <div key={i} className="bg-zinc-800/50 rounded-xl p-4 space-y-2">
                  <div className="flex justify-between items-center">
                    <span className="text-[13px] text-zinc-200 font-medium">{a.robotId}</span>
                    <span className="text-[11px] px-2.5 py-0.5 rounded-full bg-sky-500/15 text-sky-300 border border-sky-500/25">{a.role}</span>
                  </div>
                  <div className="text-[13px] text-zinc-400">{a.rationale}</div>
                  <div className="text-[11px] text-zinc-500">
            ...
            {plan.contingencies.length > 0 && (
              <div className="space-y-2.5">
                <div className="text-[11px] font-semibold text-zinc-400 uppercase tracking-wider">Contingencies</div>
                {plan.contingencies.map((c, i) => (
                  <div key={i} className="text-[13px] bg-zinc-800/50 rounded-xl p-3.5">
                    <span className="text-amber-400">If:</span>{" "}
                    <span className="text-zinc-300">{c.trigger}</span>
                    <br />
            ...
            <div className="flex justify-end gap-2 pt-3 border-t border-zinc-700/40">
              <button
                ...
                className="text-[13px] px-4 py-2 rounded-xl bg-zinc-700/60 text-zinc-300 hover:bg-zinc-600/60 transition-colors"
              >
                Reject
              </button>
              <button
